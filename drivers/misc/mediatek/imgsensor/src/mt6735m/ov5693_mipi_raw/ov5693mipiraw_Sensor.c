...
static void preview_setting(void)
{
    ...
    /********************************************************
       ...
       *
       ********************************************************/
    write_cmos_sensor(0x3820,0x46);  //v fast bin on, v flip off, v bin off
    write_cmos_sensor(0x3821,0x19);  //hsync on, mirror on, hbin on
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    if (currefps == 240) { //24fps for PIP
        ...
        write_cmos_sensor(0x3820, 0x42);//00
        write_cmos_sensor(0x3821, 0x18);//1e
        write_cmos_sensor(0x3a04, 0x06);
        write_cmos_sensor(0x3a05, 0x14);
        ...
        /********************************************************
           ...
           *
           ********************************************************/
        write_cmos_sensor(0x3820,0x42);  //v fast vbin on, flip off, v bin on//42
        if (imgsensor.ihdr_en) {

    } else {

    }
        write_cmos_sensor(0x3821,0x18);  //hsync on, h mirror on, h bin on//18
        write_cmos_sensor(0x3a04,0x06);
        write_cmos_sensor(0x3a05,0x14);
