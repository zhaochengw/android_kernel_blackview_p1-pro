...
#if 1//lisong 2015-4-13 [BUGID:NULL][backlight not be turn off in LCM OFF of engineer mode]start
        {
            unsigned long reg_base;
            
            reg_base = pwm_get_reg_base(disp_pwm_get_main());
            DISP_REG_MASK(NULL, reg_base + DISP_PWM_EN_OFF, 0x1, 0x1);
        }   
#endif//lisong 2015-4-13 [BUGID:NULL][backlight not be turn off in LCM OFF of engineer mode]end       
    }
