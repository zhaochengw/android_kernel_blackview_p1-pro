...
static long flashlight_ioctl_core(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case FLASH_IOC_IS_LOW_POWER:
            ...
            {
                int isLow=0;
#if 0//lisong 2015-3-19 [BUGID:DWYLL-37][fix bug: disable low battery protect for flashlight]start                
                if(gLowPowerPer!=BATTERY_PERCENT_LEVEL_0 || gLowPowerVbat!=LOW_BATTERY_LEVEL_0 )
                    isLow=1;
#endif//lisong 2015-3-19 [BUGID:DWYLL-37][fix bug: disable low battery protect for flashlight]end                
