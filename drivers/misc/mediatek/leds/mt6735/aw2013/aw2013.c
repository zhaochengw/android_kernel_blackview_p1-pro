...
static ssize_t aw2013_breath_show(struct device_driver *ddri, const char *buf, size_t count);
static DRIVER_ATTR(aw2013_breath,  S_IROTH  | S_IWOTH, NULL,aw2013_breath_show);
/****************************************************************************************************
** discript:read i2c
** @client :[I]: client 
** @addr   :[I]: command 
...
static void stop_led_test()
{
	char data = 0;
  char data_arr =  0x00 ;

	printk("<1> stop_led_test() start \n");
...
static int aw2013_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
		...
		g_i2c_client = client;
     printk("g_i2c_client addr = 0x%x\n",g_i2c_client->addr);
		if(err = aw2013_init_client(client))
		{
			printk("aw2013_init_client failed\n");
		...
		if((err = driver_create_file(&aw2013_i2c_driver.driver, &driver_attr_aw2013_breath)))
		{
			printk(KERN_INFO "dirver_create_file power error !\n");
		}
...
{
		printk("[zhaoshoufeng]aw2013_leds_probe\n");
...
static int __init aw2013_leds_init(void)
{
		...
		printk("aw2013 driver init start\n");
