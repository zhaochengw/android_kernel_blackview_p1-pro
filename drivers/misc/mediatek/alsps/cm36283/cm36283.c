...
#if defined(RGK_GESTURE_SUPPORT)
#define GESTURE_NAME	"gesture_key"
static struct input_dev *gesture_input_dev;
static int ps_pre_state = 1;
...
static void cm36283_eint_work(struct work_struct *work)
{
	...
	if((res = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data)))
		...
		if(sensor_data.values[0]  == 1  && ps_pre_state == 0) {	 	
    			do_gettimeofday(&tv_end);
			if(tv_end.tv_sec > tv_start.tv_sec || tv_end.tv_sec < tv_start.tv_sec)
				TimeIntervalMS = 0;// 
			else
				TimeIntervalMS = (tv_end.tv_usec - tv_start.tv_usec)/1000;
		       if (TimeIntervalMS > 0 && TimeIntervalMS < 200) {
		   //report key	
		               
		   		input_report_key(gesture_input_dev, KEY_GESTURE, 1);
				input_sync(gesture_input_dev);
				printk("luke:==============KEY_GESTURE   1==========\n");
				msleep(10);
		   		input_report_key(gesture_input_dev, KEY_GESTURE, 0);
				input_sync(gesture_input_dev);
				printk("luke:==============KEY_GESTURE   0==========\n");
		       }
			 tv_start = tv_end;
		}
		ps_pre_state = sensor_data.values[0];
#endif
#ifdef CUST_EINT_ALS_TYPE
	mt_eint_unmask(CUST_EINT_ALS_NUM);
#else
...
static int cm36283_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	gesture_input_dev->name = GESTURE_NAME;

	__set_bit(EV_KEY, gesture_input_dev->evbit);
	__set_bit(KEY_GESTURE, gesture_input_dev->keybit);
		
	err = input_register_device(gesture_input_dev);
	if (err) {
		APS_ERR( "register gesture_input_dev input device failed (%d)\n", err);
