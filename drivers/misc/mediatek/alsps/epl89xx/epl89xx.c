...
#endif
//added by lijian for factory test 20150618  start -----
static u16 ges_key_value = 0;
//added by lijian for factory test 20150618 end -----
/*----------------------------------------------------------------------------*/
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    //ges setting
    epl_sensor.ges.integration_time = EPL_GES_INTT_32;
...
#if HS_ENABLE
	//hs setting
    epl_sensor.hs.integration_time = EPL_PS_INTT_70;
	epl_sensor.hs.integration_time_max = EPL_PS_INTT_250;
	epl_sensor.hs.integration_time_min = EPL_PS_INTT_30;
...
static void epl_sensor_notify_event(int event)
{
    ...
    if(event<EVENT_UNKNOWN)
    {
        ...
        input_sync(idev);
        //added by lijian for factory test 20150618  start -----
        ges_key_value=KEYCODE_ARRAY[event];
        //added by lijian for factory test 20150618  end -----
    }
}
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_crosstalk(struct device_driver *ddri, const char *buf, size_t count)
{
...
//added by lijian for factory test 20150618  start -----
static ssize_t epl_sensor_show_elan_ges_key_value(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
		APS_LOG("ges_key_value=%d \r\n", ges_key_value);
    len += snprintf(buf+len, PAGE_SIZE-len, "%d",ges_key_value);
    return len;
}
//added by lijian for factory test end  start -----
/*----------------------------------------------------------------------------*/
...
static DRIVER_ATTR(ges_crosstalk,				S_IROTH  | S_IWOTH, NULL,   							epl_sensor_store_ges_crosstalk					);
...
//added by lijian for factory test 20150618  start -----
static DRIVER_ATTR(elan_ges_key_value,					S_IROTH  | S_IWOTH, epl_sensor_show_elan_ges_key_value,  	  		NULL										);
//added by lijian for factory test 20150618  end -----
/*----------------------------------------------------------------------------*/
...
//added by lijian for factory test 20150618  start -----    
	 &driver_attr_elan_ges_key_value,
//added by lijian for factory test 20150618  end -----	 
};
