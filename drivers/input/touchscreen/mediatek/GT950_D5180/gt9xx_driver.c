...
#endif
	//add by rgk 9375 info start
	unsigned char  tp_info[512]; 
	int len;
	//add by rgk 9375 info end
...
//#ifdef VELOCITY_CUSTOM
    ...
    tpd_load_status = 1;
	//add by rgk for 9375 info start
	sensor_id_g  =sensor_id;	
	get_module_name(sensor_id_g, module_name_g);
	#ifdef RGK_SAMPLE_VERSION
	len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x",ic_g, module_name_g,client->addr,version_info,RGK_SAMPLE_VERSION);
	#else
	len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,SW FirmWare:0x%x,",ic_g, module_name_g,client->addr,version_info);
	#endif
	fix_tp_proc_info(tp_info, len);
	//add by rgk for 9375 info end
    return 0;
}
