...
static struct class *gt9xx_class;//add by lantao 20140427
static  bool  onof=false;//add by lantao 20140227
...
#if GTP_HALL_DETECT  
extern int get_hall_state(void);
#define TPD_HALL_CHECK_CIRCLE    100
static struct delayed_work gtp_hall_check_work;
static struct workqueue_struct *gtp_hall_check_workqueue = NULL;
static void gtp_hall_check_func(struct work_struct *);
static u8 gtp_hall_mode = 0;
#endif
...
#if GTP_HALL_DETECT //add by lantao 
...
//Start ==== lantao
static u32 lcd_heigh;
extern UINT32 DISP_GetScreenHeight(void);
//End === lantao
#if GTP_GT9XX_GT9157_CFG_SUPPORT
static u32 icVersion=0;
static s32 gtp_init_panel_gt9157(struct i2c_client *client);
static s32 gtp_init_panel_qhd_gt9157(struct i2c_client *client);
#endif
static int gt91xx_config_read_proc(struct file *, char __user *, size_t, loff_t *);
static int gt91xx_config_write_proc(struct file *, const char __user *, size_t, loff_t *);
static struct proc_dir_entry *gt91xx_config_proc = NULL;

//add by lantao start 20150427
enum{
	GESTURE_OFF = 0,
	GESTURE_ON,
};

static ssize_t enable_gt9xx_show(struct class *dev, 
	struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "lantao--> %s\n", "gesenable");
}

static ssize_t enable_gt9xx_store(struct class *class, 
...
static CLASS_ATTR(gesenable, 0666, enable_gt9xx_show, enable_gt9xx_store);
//add by lantao end   20150424
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    {	
        ...
        if (gtp_hall_mode == 1)
        {
            GTP_DEBUG("Write hall config");
            ret = gtp_i2c_write(client, config_hall , GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        }
        else
        {
...
#if GTP_GT9XX_GT9157_CFG_SUPPORT	
    if(buf[5]=='7'&&buf[4]=='5'&&buf[3]=='1'&&buf[2]=='9')
    	{
    		icVersion = 9157;
    	}
#endif	
...
Output:
    Executive outcomes.0---succeed.
*******************************************************/
extern int gt9xx_sensor_id;
static s32 gtp_init_panel(struct i2c_client *client)
{
    s32 ret = 0;
#if GTP_HALL_DETECT //luojy
    u8 cfg_info_group1_hall[] = CTP_CFG_GROUP1_HALL;
    u8 cfg_info_group2_hall[] = CTP_CFG_GROUP2_HALL;
    u8 cfg_info_group3_hall[] = CTP_CFG_GROUP3_HALL;
    u8 *send_cfg_buf_hall[3] = {cfg_info_group1_hall, cfg_info_group2_hall, cfg_info_group3_hall};
#endif	
#if GTP_DRIVER_SEND_CFG
    s32 i;
    u8 check_sum = 0;
...
//    gt9xx_sensor_id=sensor_id;
 ...
 //           }
  ...
  //          }
    ...
    memcpy(&config_hall[GTP_ADDR_LENGTH], send_cfg_buf_hall[sensor_id], cfg_len);
#endif	
#if GTP_CUSTOM_CFG
	...
	for (i = GTP_ADDR_LENGTH; i < cfg_len; i++)    
		{       
			check_sum += config_hall[i];   
		}    
	config_hall[cfg_len] = (~check_sum) + 1;
#endif  
    ...
    {
    #if GTP_DRIVER_SEND_CFG
//    gt9xx_sensor_id=cfg_info_group1[1];
...
#ifdef RGK_TP_COMPALITE_LCM // -------- fangxing add for tp upgrade begin ------
static s32 gtp_init_panel_qhd(struct i2c_client *client)
{
    s32 ret = 0;
#if GTP_HALL_DETECT //luojy
    u8 cfg_info_group1_hall[] = CTP_CFG_GROUP1_QHD_HALL;
    u8 cfg_info_group2_hall[] = CTP_CFG_GROUP2_HALL;
    u8 cfg_info_group3_hall[] = CTP_CFG_GROUP3_HALL;
    u8 *send_cfg_buf_hall[3] = {cfg_info_group1_hall, cfg_info_group2_hall, cfg_info_group3_hall};
#endif	
    ...
    u8 cfg_info_group1[] = CTP_CFG_GROUP1_QHD;
    u8 cfg_info_group2[] = CTP_CFG_GROUP2;
    u8 cfg_info_group3[] = CTP_CFG_GROUP3;
    u8 cfg_info_group4[] = CTP_CFG_GROUP4;
    u8 cfg_info_group5[] = CTP_CFG_GROUP5;
    ...
    else
    {
        ...
        GTP_INFO("GT9XX_2 Sensor_ID: %d", sensor_id);
    }
 //   gt9xx_sensor_id=sensor_id;
    cfg_len = cfg_info_len[sensor_id];
...
#endif 
    if(tpd_load_status){
    	printk("fangxing: gt9xx probe FAIL! %s  %d \n",__func__,__LINE__);
		return -1;
	}
#if GTP_SUPPORT_I2C_DMA
...
//	tpd_v_magnify_y = TPD_VELOCITY_CUSTOM_Y;
...
#ifdef RGK_TP_COMPALITE_LCM //fangxing begin
    if (lcd_heigh == 1280)
	{
	#if GTP_GT9XX_GT9157_CFG_SUPPORT		
		if(icVersion==9157)
		{
			ret = gtp_init_panel_gt9157(client);
		}
		else
	#endif	 	
		{
	...
	else
    {
		...
		if(icVersion==9157)
		{
			ret = gtp_init_panel_qhd_gt9157(client);
			printk("GTP_INFO:GT9157\n");
		}
		else
#endif			
		{
			ret = gtp_init_panel_qhd(client);
			printk("*******************************\n");
		}
    }
#else
	...
	gt9xx_class = class_create(THIS_MODULE, "syna");
	ret = class_create_file(gt9xx_class, &class_attr_gesenable);
	if (ret) {
		printk("gt9xx gensenable create sys interface ERROR\n");
	}
...
#endif
#if GTP_GESTURE_WAKEUP
#ifdef RGK_GESTURE_WAKEUP_KEY_CUSTOMIZE//add by fangxing for gesture wakeup key report
    input_set_capability(tpd->dev, EV_KEY, KEY_RIGHT);
//del by lzj    input_set_capability(tpd->dev, EV_KEY, KEY_UP);
...
#if GTP_HALL_DETECT  //luojy
    INIT_DELAYED_WORK(&gtp_hall_check_work, gtp_hall_check_func);
    gtp_hall_check_workqueue = create_workqueue("gtp_hall_check");
    queue_delayed_work(gtp_hall_check_workqueue, &gtp_hall_check_work, TPD_HALL_CHECK_CIRCLE);
#endif
    tpd_load_status = 1;
...
#endif
#if GTP_HALL_DETECT  //luojy
static void gtp_hall_check_func(struct work_struct *work)
{
    int cur_hall_state =0;
    cur_hall_state =get_hall_state();

    GTP_DEBUG("Hall mode = %d", cur_hall_state);

    if (gtp_hall_mode != cur_hall_state)
    {
        GTP_DEBUG("Hall state change detected~!");
        GTP_DEBUG("Hall mode = %d", cur_hall_state);
        gtp_hall_mode = cur_hall_state;
...
//    #if GTP_GESTURE_WAKEUP
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {     
                if ( (doze_buf[2] == 'c') || (doze_buf[2] == 'e') || (doze_buf[2] == 'm') ||
					 ...
					 (doze_buf[2] == 'w') || (doze_buf[2] == 'z') /*|| (doze_buf[2] == 0x5E)*/  /* ^ */
                    )				
                {
					switch(doze_buf[2])
					{					
	...
	/*					case 0X5E:
						{
							doze_status = DOZE_WAKEUP;
							input_report_key(tpd->dev, 0X5E, 1);
							input_sync(tpd->dev);
							input_report_key(tpd->dev, 0X5E, 0);
							input_sync(tpd->dev);
							// clear 0x814B
							doze_buf[2] = 0x00;
							gtp_i2c_write(i2c_client_point, doze_buf, 3);
						...
						default:
						  ...
						  (doze_buf[2] == 0xAB))//delete slide up sxl || (doze_buf[2] == 0xBA)
                {
                    char *direction[3] = {"Right", "Down", "Left"};
...
//#endif
#if GTP_HALL_DETECT //luojy
    cancel_delayed_work_sync(&gtp_hall_check_work);
#endif
    mutex_unlock(&i2c_access);
...
static int __init tpd_driver_init(void)
{
    ...
    lcd_heigh =DISP_GetScreenHeight();//add by fangxing	
...
#if GTP_HALL_DETECT //luojy
static s32 tpd_hall_funtion(int ishallenable)
{
 s32 ret = 0;	
if(ishallenable) //hall close,hall enable
{
    ...
    if (ret < 0)
    {
        GTP_ERROR("Send  Hall config error.");
        goto out;
    }

     GTP_DEBUG("Write hall config Success!");
...
{       
    ...
    if (ret < 0)
    {
        GTP_ERROR("Send normal config error.");
        goto out;
    }

      GTP_DEBUG("Write normal config Success!");
...
#if GTP_HALL_DETECT //added by lijian
extern int get_hall_state(void)
{
	int ret = (int )mt_get_gpio_in(GPIO_MHALL_EINT_PIN) ;
	if(ret==0)
		{
			return 1;
		}
...
static s32 gtp_init_panel_gt9157(struct i2c_client *client)
{
    s32 ret = 0;
#if GTP_HALL_DETECT //luojy
    u8 cfg_info_group1_hall[] = CTP_CFG_GROUP1_GT9157_HALL;
    u8 cfg_info_group2_hall[] = CTP_CFG_GROUP2_HALL;
    u8 cfg_info_group3_hall[] = CTP_CFG_GROUP3_HALL;
    u8 *send_cfg_buf_hall[3] = {cfg_info_group1_hall, cfg_info_group2_hall, cfg_info_group3_hall};
#endif	
    ...
    u8 cfg_info_group1[] = CTP_CFG_GROUP1_GT9157;
    u8 cfg_info_group2[] = CTP_CFG_GROUP2;
    u8 cfg_info_group3[] = CTP_CFG_GROUP3;
    u8 cfg_info_group4[] = CTP_CFG_GROUP4;
    u8 cfg_info_group5[] = CTP_CFG_GROUP5;
...
static s32 gtp_init_panel_qhd_gt9157(struct i2c_client *client)
{
    s32 ret = 0;
#if GTP_HALL_DETECT //luojy
    u8 cfg_info_group1_hall[] = CTP_CFG_GROUP1_QHD_GT9157_HALL;
    u8 cfg_info_group2_hall[] = CTP_CFG_GROUP2_HALL;
    u8 cfg_info_group3_hall[] = CTP_CFG_GROUP3_HALL;
    u8 *send_cfg_buf_hall[3] = {cfg_info_group1_hall, cfg_info_group2_hall, cfg_info_group3_hall};
#endif		
    ...
    u8 cfg_info_group1[] = CTP_CFG_GROUP1_QHD_GT9157;
    u8 cfg_info_group2[] = CTP_CFG_GROUP2;
    u8 cfg_info_group3[] = CTP_CFG_GROUP3;
    u8 cfg_info_group4[] = CTP_CFG_GROUP4;
    u8 cfg_info_group5[] = CTP_CFG_GROUP5;
